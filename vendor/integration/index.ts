  ...
  return {
    ...
    hooks: {
      'astro:config:setup': async ({
        ...
        const buildLogger = logger.fork('fadetechs');

        const virtualModuleId = 'fadetechs:config';
        ...
        updateConfig({
          ...
          vite: {
            plugins: [
              {
                name: 'vite-plugin-fadetechs-config',
                resolveId(id) {
        ...
        if (typeof _themeConfig === 'string') {
          ...
          buildLogger.info(`Fade Technologies \`${_themeConfig}\` has been loaded.`);
        } else {
          buildLogger.info(`Fade Technologies config has been loaded.`);
        }
      },
